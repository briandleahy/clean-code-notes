...
Names
-----
1.  *It's hard to overestimate the value of good names.*
...
4.  *Make meaningful distinctions in variable names.*
...
9.  *Class names should be nouns or noun phrases.*
9.  *Method names should be verbs or verb phrases.*
    Accessors, mutators, and predicates should be named for the value and prefixed with `get`, `set`, and `is`.
10. *For functions which take arguments, the function name and argument should make a verb-object pair.*
...
2. *Functions should do 1 thing and 1 thing only.*
    * Corollary: `if`, `else`, `while`, `for`, etc blocks should be 1 line long, calling an additional sub-method.
...
4.  *The fewer arguments to a function, the better.*
    The ideal number of arguments to a function is 0, followed by 1 and 2. Three arguments should be avoided wherever possible. Four or more arguments is a code smell. Why?
    * It's hard to remember which argument is which -- is it `copy(src, dest)` or `copy(dst, src)`?
    * When you test, you need to have a combinatoric number of tests to capture all the possible behavior.
...
1.  *Comments are a necessary evil.*
    Comments compensate for a failure to express yourself in code. It's better to write clean, expressive code than to have abstruse code with clarifying comments.
...
2.  *Keep the tests clean and readable.*
    The tests need to follow the same high standards for cleanliness as the rest of the code. Otherwise, when the code changes, it will be difficult to change the tests to keep up.
...
4.  *Tests should be fast.*
    They should run quickly enough where you can run them all the time. Try to keep the entire test suite running in less than a few seconds.
...
3.  *Classes should have a small number of instance variables.* Otherwise the class is probably not doing one thing.
